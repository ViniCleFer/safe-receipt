...
export default function Enunciado() {
  ...
  const { id, grupo } = useLocalSearchParams<{
    id: string;
  ...
  async function loadEnunciados() {
    ...
    try {
      const response = await getDetailsEnunciadoRequest(id);
      ...
      if (response?.status === 200) {
        ...
        setEnunciadoFounded(response?.data);
      }
    ...
    try {
      ...
      if (response?.status === 201) {
        ...
        if (thereIsNextEnunciado) {
          ...
          push(
            `/(home)/enunciado/${proximoEnunciadoId}?grupo=${enunciadoFounded?.grupo}`
          );
        } else {
